...
Roles referenced below: RAID member disks (data), the root drive, and the USB
backup drive. Device letters (`sdX`) are **not** assumed stable — they reorder
across reboots — so every step selects drives by hardware identity (serial /
by-id / RAID membership / transport), never by letter. Where a letter appears in a
command it's only the *current* name you look up live.
...
Generate explicit by-id device lines for the **internal (SATA) drives only**. Select
them by transport — `sata` = internal (root + array), `usb` = the backup drive — so
the selection can't grab the backup drive or miss an internal one when letters
reorder. (Note this is *not* RAID-membership: smartd should still monitor the root
drive, which isn't an array member.)

```bash
OPTS='-a -o on -S on -n standby,q -s (S/../.././02|L/../../6/03) -W 4,50,55 -m root -M exec /usr/local/bin/smartd-notify.sh'
lsblk -dno NAME,TRAN                                   # sanity: confirm which disk is usb (the backup)
lsblk -dno NAME,TRAN | awk '$2=="sata"{print $1}' | while read -r d; do
  id=$(udevadm info -q symlink -n "/dev/$d" | tr ' ' '\n' | grep -m1 '^disk/by-id/ata-') \
    || id=$(udevadm info -q symlink -n "/dev/$d" | tr ' ' '\n' | grep -m1 '^disk/by-id/wwn-')
  [ -n "$id" ] && echo "/dev/$id $OPTS"
done
```

Edit `/etc/smartd.conf`: **comment out the `DEVICESCAN` line** and paste the
generated by-id lines in its place. Then reload and verify:
